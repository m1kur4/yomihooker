...
async function main() {
  ...
  for (const deck of decks) {
    ...
    const messagesPath = join(process.cwd(), "data", "decks", String(deck.id), "messages.json");
    if (!existsSync(messagesPath)) {
      console.log(`    No messages file for deck ${deck.id}, skipping messages.`);
      continue;
    }

    const messages = JSON.parse(readFileSync(messagesPath, "utf-8")) as MessageJson[];
